    ...
    try {
      ...
      if (url.includes('mock-') || url.includes('/mock') || !url.startsWith('http')) {
        const mockPrices: Record<string, { title: string; price: number }> = {
          'mock-iphone15': { title: 'Apple iPhone 15 (Black, 128 GB)', price: 65999 },
          'mock-macbookm3': { title: 'Apple MacBook Air M3 (13.6-inch, 8GB RAM, 256GB SSD)', price: 104990 },
          'mock-sonywh5': { title: 'Sony WH-1000XM5 Bluetooth Headset with Active Noise Cancellation', price: 29990 },
          'mock-s24u': { title: 'SAMSUNG Galaxy S24 Ultra (Titanium Gray, 256 GB)', price: 124999 },
          'mock-sony-wf': { title: 'Sony WF-1000XM5 Wireless Noise Cancelling Earbuds', price: 19990 },
          'mock-ipad-m4': { title: 'Apple iPad Pro M4 (11-inch, 256GB, Wi-Fi)', price: 87900 }
        };

        let matched = { title: 'Mock Flipkart Product', price: 45999 };
      ...
      // Extract Title
      const titleMatch = html.match(/<h1[^>]*class=["'][^"']*VU-ZEz[^"']*["'][^>]*>([\s\S]*?)<\/h1>/i) ||
      ...
      let price = 0;
      const patterns = [
        /<div[^>]*class=["'][^"']*Nx9bqj[^"']*["'][^>]*>[^\d]*([\d,]+)/i,
        /<div[^>]*class=["'][^"']*_30jeq3[^"']*["'][^>]*>[^\d]*([\d,]+)/i,
        /<span[^>]*class=["'][^"']*_30jeq3[^"']*["'][^>]*>[^\d]*([\d,.]+)/i
      ];
